...
This dataset uses data from the [Gapminder Foundation](https://www.gapminder.org/). We're downloading the `gapminder_1950_2018.csv` dataset into our project, which contains a small fraction of the available data and has been cleaned up make it easy to work with for the purposes of this course.

1. Create a `data` folder in your working directory.
2. Download the dataset into the `data` folder by copying and running in the `Console` (no need to run it in your R Notebook, otherwise the data get downloaded every time):
...
The dataset contains data for various countries across the world. We will be doing some data wrangling, so that we can focus on a subset of these.
